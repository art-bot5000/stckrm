// ═══════════════════════════════════════════════════════════════════
//  NOTES — extracted from app.js (Option C plain-script split)
// ═══════════════════════════════════════════════════════════════════
//
// This file contains the entire Secure Notes feature: state, persistence,
// editor, sharing-into-notes plumbing, lock/unlock, colour swatches,
// undo/redo, and the linked-reminder bridge.
//
// Loading model: this is a PLAIN <script>, not type="module". It's loaded
// in index.html AFTER app.js with `defer`, so by the time these top-level
// declarations execute, app.js has already finished defining the globals
// this file depends on (toast, esc, postKV, _kvSessionToken, reminders,
// saveReminders, _shareState, etc).
//
// Outbound dependencies on app.js (all accessed as globals):
//   UI:        toast, toastAction, esc, toggle, show, hide,
//              openModal, closeModal, fmt, fmtDate, uid
//   Auth/sync: WORKER_URL, _kvSessionToken, _kvVerifier, _kvEmailHash,
//              _kvKey, postKV, kvEnsureKey, kvEncrypt, kvDecrypt, kvPush,
//              kvConnected, _readJsonSafe, dbGet, dbPut
//   Reminders: reminders[], saveReminders, getReminderDaysUntil
//   Shares:    _shareState, isOwner, openSharingPanelFor,
//              registerSharingSection
//   MFA:       _mfaEnabled, _maybeShowMfaPrompt, requireReauth
//   Misc:      MS_PER_DAY
//
// Inbound API exposed to app.js (all globals, called as such from app.js):
//   State:     notes[], _noteUnlocked, _sharedNotesIncoming
//   Lifecycle: loadNotes, saveNotes
//   UI:        renderNotes, openNoteEditor, _relockAllNotes
//   Helpers:   _noteUid, _syncNoteIfConnected, _noteBgForCurrentTheme
//
// History: extracted on 2026-05-21. Prior to this, all of the below lived
// in app.js across four discontiguous regions (state ~L2249, persistence
// ~L4361, theme helper ~L22226, main block ~L29514). The split mirrors
// the budget.js extraction pattern from 2026-05-20.
